...
export function createBot(token, options = {}) {
    ...
    async function formFill(conversation, ctx) {
        ...
        labelsKeyboardName.push(ctx.from?.first_name);
        ...
        console.log(name);
        ctx.reply(`Расскажи о себе и кого хочешь найти, чем предлагаешь заняться. Это поможет лучше подобрать тебе компанию.`, {
            reply_markup: keyboardDescriprion,
        });
        const description = await conversation.form.text();
        console.log(description);
        await ctx.reply(`Теперь пришли фото или запиши видео 👍 (до 15 сек), его будут видеть другие пользователи`, {
            reply_markup: { remove_keyboard: true },
        });
        ...
        await ctx[isVideoMedia ? "replyWithVideo" : "replyWithPhoto"](ctx.session.myProfile.media, {
    ...
    async function formFillAgain(conversation, ctx) {
        ...
        ctx.reply(`Теперь пришли фото или запиши видео 👍 (до 15 сек), его будут видеть другие пользователи`, {
        ...
        await conversation.external(async () => {
            const newProfile = await prisma.profile.update({
                ...
                data: {
                    ...
                    description: description === "Пропустить" ? "" : description,
                    media: mediaValue,
                    platformId: ctx.session.myProfile.platformId,
                },
            });
            ctx.session.myProfile = newProfile;
        });
    ...
    async function profileMain(conversation, ctx) {
        await ctx.reply("1. Заполнить анкету заново. \n2. Изменить фото/видео. \n3. Изменить текст анкеты. \n4. Смотреть анкеты.", { reply_markup: keyboardProfile });
    ...
    async function showNewProfiles(conversation, ctx) {
        for (let i = 0; i < ctx.session.profiles.length; i++) {
            const getMedia = await ctx.api.getFile(ctx.session.profiles[i].media);
            const isVideoMedia = getMedia.file_path.includes("videos");
            await ctx[isVideoMedia ? "replyWithVideo" : "replyWithPhoto"](ctx.session.profiles[i].media, {
                reply_markup: keyboardRate,
                caption: `${ctx.session.profiles[i].name}, ${ctx.session.profiles[i].age}, ${ctx.session.profiles[i].city}  ${ctx.session.profiles[i].description
            ...
            if (rate === "❤️") {
                ...
                await ctx.api.sendMessage(Number(ctx.session.profiles[i].platformId), "Вы кому-то понравились!", { reply_markup: keyboardRate });
    ...
    const keyboardStop = Keyboard.from(buttonRowsStop)
        ...
        .resized()
        .oneTime();
    async function stopShow(conversation, ctx) {
        await ctx.reply("Подождем пока кто-то увидит твою анкету");
        await ctx.reply("1. Смотреть анкеты. \n2. Моя анкета. \n3. Я больше не хочу никого искать.", { reply_markup: keyboardStop });
    ...
    const main = async (ctx) => {
        ...
        const isVideoMedia = getMedia.file_path.includes("videos");
        await ctx[isVideoMedia ? "replyWithVideo" : "replyWithPhoto"](ctx.session.myProfile.media, {
            caption: `${ctx.session.myProfile.name}, ${ctx.session.myProfile.age}, ${ctx.session.myProfile.city} ${ctx.session.myProfile.description
                ? "- " + ctx.session.myProfile.description
                : ""}`,
        });
        ctx.session.profiles = await prisma.profile.findMany({
            where: {
                sex: ctx.session.myProfile?.interest === 3
                    ? undefined
